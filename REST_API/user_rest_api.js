...
REST_ROUTER.prototype.handleRoutes= function(router,connection,md5) {
    ...
    router.post("/users/address/:user_id", function(req, res){
     ...
     router.post("/users/check_login",function(req,res){
        ...
        var table = ["login", "username",req.body.username, "password", md5(req.body.password)];
    ...
    */
    ...
    router.put('/users/login_status/:user_info_id/:login_status', function(req, res){
       var query = "UPDATE ?? SET ?? = ? WHERE ?? = ?";
        var table = ["login", "login_status", req.params.login_status, "user_info_id", req.params.user_info_id];
        query = mysql.format(query,table);
        connection.query(query,function(err,rows){
            if(err) {
                res.json({"Error" : true, "Message" : "Error executing MySQL query"});
